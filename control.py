...
class Game:
    ...
    def __init__(self):
        ...
        self.font_title = choose_font(['tahoma', 'arial', 'dejavusans'], 42, bold=True)
        self.font_h2 = choose_font(['tahoma', 'arial', 'dejavusans'], 31, bold=True)
        self.font_btn = choose_font(['trebuchetms', 'tahoma', 'arial', 'dejavusans'], 24, bold=True)
    ...
    def menu_buttons(self):
        cx = WINDOW_WIDTH // 2
        return [Button('pvp', pygame.Rect(cx - 150, 210, 300, 56), 'Два игрока', 'green'), Button('pve', pygame.Rect(cx - 150, 284, 300, 56), 'Против компьютера', 'orange'), Button('easy', pygame.Rect(cx - 180, 392, 110, 48), 'Легко', 'green'), Button('medium', pygame.Rect(cx - 55, 392, 110, 48), 'Средне', 'yellow'), Button('hard', pygame.Rect(cx + 70, 392, 110, 48), 'Сложно', 'red')]

    def setup_buttons(self):
        panel_x = LEFT_MARGIN + BOARD_SIZE + 28
        panel_w = 360
        base_y = TOP_MARGIN + 280
        return [Button('start', pygame.Rect(panel_x, base_y, panel_w, 56), 'Начать игру', 'green'), Button('random', pygame.Rect(panel_x, base_y + 72, 172, 52), 'Случайно', 'orange'), Button('clear', pygame.Rect(panel_x + panel_w - 172, base_y + 72, 172, 52), 'Очистить', 'purple')]
