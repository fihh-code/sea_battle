...
class GameUI:
    ...
    def draw_setup(self):
        ...
        rules_cont = self.assets.draw_headered_panel(self.screen, rules_panel, 'ПРАВИЛА', self.font_text, TEXT_TITLE, TEXT_SHADOW)
        rules = ['R — повернуть корабль', 'ЛКМ — поставить корабль', 'Клик по резерву — выбрать', 'Случайно — авторасстановка']
        ...
        res_content = self.assets.draw_headered_panel(self.screen, res_panel, 'РЕЗЕРВ', self.font_text, TEXT_TITLE, TEXT_SHADOW)
    ...
    def draw_icons(self, content_rect, live_ships, cache_pref):
        ...
        for ship in sorted(live_ships, key=lambda s: (-s.size, s.positions[0][1], s.positions[0][0])):
            icon = self.assets.scaled(f'{cache_pref}_{ship.size}', self.assets.ship_h[ship.size], (ship.size * 17, 15))
